...
export default async function handler(
  ...
  try {
    ...
    let storyContent =
      `Create a fairy tale typical for the country specified by the ${region}.\n` +
    ...
    storyContent +=
      "- Write the story in both English and the native language of ${region}, with the native language version starting with the words: ${region} Language.\n" +
    ...
    const story = JSON.parse(groqData.choices[0].message.content);

    if (!story.title) {
      return res.status(500).json({ error: "Story title is missing" });
    }
